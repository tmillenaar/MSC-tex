...
\begin{figure*}
\centering
\includegraphics[width=1.0\textwidth,keepaspectratio]{setup-schematic.png}
\caption{Setup for the numerical model where $U_{lat}$ is the lateral velocity in and out of the marginal basin, $Z_{strait}$ is the depth of the strait connecting the basins, $Z_u$ is the flowdepth or the vertical extent of the exchanging flow, S represents the source-sink term as shown in equation \ref{eq:Source-Sink} and W represents the advection.}
\label{fig:setup}
\end{figure*}
...
\begin{equation}
    S = \pm Sal_{source} \frac{ z_{strait} x_{strait} U_{lat} }{A_{sink} z_u} 
    \label{eq:Source-Sink}
\end{equation}
Where \textbf{Sal}$_{source}$ is the salinity of the source in $kg \hspace{0.8mm} m^{-3}$, \textbf{U}$_{lat}$ is the lateral flow velocity in $ms^{-1}$, $A_{sink}$ is the surface area of the sink basin in $m^2$, \textbf{z}$_u$ is the vertical extent of the flow in $m$ and \textbf{x}$_{strait}$ and \textbf{z}$_{strait}$ are the strait width and depth, respectively, in $m$.
...
\subsection{The transition period as an example of the differences between the two mothods to check instability}
\label{sect:transition}
\subsubsection{Results of the transition case}
The transition around the tenth year is shown in in figure~\ref{fig:Transition}. From April year 10, mixing is turned on. Around September, the top node is the same density as the one below it. From this moment onward, mixing takes place. In October, the top 30 or so metres are mixing, after which this is the case for the entire column. From march onward the mixing has stopped. This pattern of mixing only in winter results in an asymmetric profile, as shown in figure~\ref{fig:Conv_Only}. During the winter mixing, the profile is almost straight, though slightly inclined due to the finiteness of the diffusivity constant. From April until September, figure \ref{fig:Transition} resembles figure \ref{fig:SpinUp}. As the fall progresses, an unstable column develops at the surface and increases in vertical extend as the surface water cools. 
...
In order to obtain a dense saline brine at the bottom of the deep basin, the bottom of the marginal basin clearly needs to be significantly more saline then is achieved through the parameters used in the runs shown so far, for in this model a saline bottom of the  marginal basin is directly related to a more saline bottom of the deep basin. This could be done by increasing the total amount of salt in the marginal basin through net evaporation or by allowing for faster transport of salt to the bottom of the marginal basin. The latter can be achieved by decreasing the basin depth or increasing the amount of diffusion during mixing. A combination of these is likely required to achieve the sought effect. These factors will be discussed in section \ref{sect:imprtant_parameters}.
...
\subsubsection{Extent of the salt brine}
